...
<section class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <nav class="flex items-center gap-2 text-sm text-slate-500">
            ...
            <a href="{{ route('themes.show', $theme) }}" class="hover:text-rose-600 transition-colors">{{ $theme->name }}</a>
            <i data-lucide="chevron-right" class="w-4 h-4"></i>
            <span class="font-semibold {{ $s['text'] }}">{{ $level }}</span>
        </nav>

        <!-- Level switcher -->
        <div class="inline-flex items-center rounded-xl bg-white border border-slate-200 p-1 shadow-sm">
            @foreach (['A1','A2','B1','B2'] as $lvl)
                @php $ls = $levelStyles[$lvl]; @endphp
                <a href="{{ route('game.play', [$theme, strtolower($lvl)]) }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all {{ $level === $lvl ? 'bg-gradient-to-r '.$ls['gradient'].' text-white shadow' : 'text-slate-500 hover:bg-slate-100' }}">
                    {{ $lvl }}
                </a>
            @endforeach
        </div>
    ...
    <div class="flex items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="font-display font-extrabold text-xl sm:text-2xl text-slate-800 leading-tight">{{ $theme->name }}</h1>
                <p class="text-sm text-slate-500">{{ $theme->name }} · {{ $level }} deck · {{ $questions->count() }} cards</p>
            </div>
        </div>

        <!-- Timer -->
        <div class="flex items-center gap-3">
            <div id="timerCard" class="flex items-center gap-2 px-4 py-2 rounded-2xl bg-white border border-slate-200 shadow-sm">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between text-sm mb-1.5">
            <span class="font-semibold text-slate-600">Card <span id="currentNum" class="{{ $s['text'] }}">1</span> of <span id="totalNum">{{ $questions->count() }}</span></span>
            <span class="text-slate-400">Practiced: <span id="practicedCount" class="font-bold {{ $s['text'] }}">0</span></span>
        </div>
    ...
    <div class="relative mx-auto max-w-2xl mb-6" style="perspective: 1600px;">
        <div id="card" class="relative w-full cursor-pointer transition-transform duration-500" style="transform-style: preserve-3d; min-height: 420px;">
            ...
            <div class="card-face absolute inset-0 rounded-3xl bg-gradient-to-br {{ $theme->gradient }} p-[2px] shadow-2xl" style="backface-visibility: hidden;">
                <div class="relative h-full rounded-[22px] bg-white p-8 sm:p-10 flex flex-col">
                    ...
                    <div class="flex-1 flex flex-col items-center justify-center text-center">
                        ...
                        <p id="promptText" class="font-display font-bold text-2xl sm:text-3xl text-slate-800 leading-snug max-w-lg"></p>
                    </div>
            ...
            <div class="card-face absolute inset-0 rounded-3xl bg-gradient-to-br {{ $s['gradient'] }} p-[2px] shadow-2xl" style="backface-visibility: hidden; transform: rotateY(180deg);">
                <div class="relative h-full rounded-[22px] bg-white p-8 sm:p-10 flex flex-col overflow-hidden">
                    ...
                    <div class="relative flex items-center justify-between mb-3">
                        ...
                        <button type="button" id="flipBackBtn" class="grid place-items-center w-8 h-8 rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors" aria-label="Flip back">
                            <i data-lucide="rotate-ccw" class="w-4 h-4"></i>
                        </button>
                    </div>

                    <div class="relative flex-1 overflow-y-auto custom-scroll pr-1">
                        <p id="answerText" class="text-base text-slate-700 leading-relaxed"></p>
                        ...
                        <div id="tipsBox" class="mt-5 rounded-2xl {{ $s['soft'] }} border {{ $s['border'] }} p-4">
                            ...
                            <p id="tipsText" class="text-sm text-slate-600"></p>
                        </div>

                        <div id="vocabBox" class="mt-4">
                            <p class="text-xs font-bold uppercase tracking-wide text-slate-400 mb-2 flex items-center gap-1.5"><i data-lucide="book-open" class="w-3.5 h-3.5"></i> Useful vocabulary</p>
    ...
    <div class="flex flex-wrap items-center justify-center gap-3">
        <button id="prevBtn" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 font-semibold shadow-sm hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
            <i data-lucide="arrow-left" class="w-4 h-4"></i> Prev
        </button>

        <button id="shuffleBtn" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 font-semibold shadow-sm hover:bg-slate-50 transition-all">
            <i data-lucide="shuffle" class="w-4 h-4"></i> Shuffle
        </button>

        <button id="practicedBtn" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 font-semibold shadow-sm hover:bg-slate-50 transition-all">
        ...
        <button id="nextBtn" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 font-semibold shadow-sm hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
    ...
    <div class="mt-10 text-center">
        <a href="{{ route('themes.show', $theme) }}" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-rose-600 transition-colors">
